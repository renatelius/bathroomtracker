...
## Завершено: Progressive-онбординг (пошаговый)
- **`src/screens/Onboarding.js`** переписан в многошаговый flow (4 шага): 1) Пол, 2) Рост/вес/год рождения, 3) Тип телосложения, 4) Сводка/«Завершить».
- Прогресс-бар из 4 сегментов по верхнему краю; кнопки «Назад»/«Далее» (Далее только при валидном шаге); на финальном шаге — сводка выбранных данных перед сохранением.
- Тот же профиль сохраняется через `saveProfile` → `onDone`; ничего не отправляется; подсказки по валидации на каждом шаге.
- Стиль согласован с остальной системой (хук `useThemeColors`, светлая/тёмная тема, `Chip`/`TextField`/`ScreenHeader`/`Button`).
- Проверки: `npm test` 19/19, Babel OK, `expo export --platform web` OK; строки шагов/кнопок подтверждены в dev-бандле.
