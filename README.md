...
- **Лог еды** — поиск блюд через Open Food Facts API (с картинками), калории из
  `nutriments.energy-kcal_100g`, выбор веса порции.
- **Фото приёма пищи** — загрузка фото блюда, ручной ввод калорий и демо-оценка
  калорий по фото (`src/services/vision.js`, mock-заглушка под реальный Vision/LLM).
...
- **Профиль** — аватар (фото/инициалы), данные, выбор языка.
- **Языки (i18n)** — 5 языков (ru/en/es/de/fr), автоопределение по региону
  устройства, ручной выбор в Профиле.
...
- **Дизайн-система** — единая палитра (спокойный беж/бирюза, акцент `#2F7D63`),
  токены и переиспользуемые компоненты в `src/theme/` и `src/ui/`.
